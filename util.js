...
var util = {
  ...
  parseQueryString: function(url) {
    ...
    var paramStr = url.substr(url.lastIndexOf('?') + 1);
    return JSON.parse(
      '{"' +
  ...
  /**
   * 封装ajax
   * @param  {Object} options 传入封装数据的对象
   */
  getJson: function(options) {
    ...
    if (opt.url) {
      ...
      xhr.loadstart = function() {
        if (opt.beforeSend && opt.beforeSend instanceof Function) {
          opt.beforeSend.call(xhr);
        }
      };
      ...
      xhr.onload = function() {
        if ((xhr.status >= 200 && xhr.status < 300) || xhr.status === 304) {
          ...
          if (opt.error && opt.error instanceof Function) {
            opt.error.call(xhr, xhr.status, xhr.statusText);
          }
        }
      };
